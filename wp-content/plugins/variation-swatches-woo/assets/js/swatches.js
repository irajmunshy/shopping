(function($) {
    ...
    const SW = {
                ...
                if ('' !== getAttrName) {
                    ...
                    findOptions.each(function() {
                        ...
                        if (currentTermSlug && '' !== currentTermSlug) {
                            const hasStock = SW.checkOptionAvail(getAttrName, currentTermSlug, getSelectedOptions, getFormData);
                            if (!hasStock) {
        ...
        checkOptionAvail: (getAttrName, currentTermSlug, getSelectedOptions, getFormData) => {
            let hasThisSwatch;
            for (let index = 0; index < getFormData.length; index++) {
                ...
                const passedInRaw = SW.checkInPreRawData(getSelectedOptions, attributes, getAttrName, currentTermSlug, is_in_stock);
                if (passedInRaw) {
        ...
        checkInPreRawData: (getSelectedOptions, attributes, getAttrName, currentTermSlug, is_in_stock) => {
            ...
            hasThisSwatch = SW.checkInRawData(attributes, copySelected, currentObj, is_in_stock);
            return hasThisSwatch;
        },
        ...
        Events: () => {
            $(document).on('click', '.cfvsw-swatches-container .cfvsw-swatches-option[data-slug]', SW.chooseOption);
        },
    };
    ...
    $('body').on('click', '.cfvsw_ajax_add_to_cart.cfvsw_variation_found', function(e) {
        e.preventDefault();
    ...
    function updateSelectOption(swatch) {
        const value = swatch.hasClass('cfvsw-selected-swatch') ? swatch.data('slug') : '';
        const select = swatch.closest('.cfvsw-swatches-container').prev().find('select');
    ...
    function updateTitle(swatch) {
        ...
        label.children('label').append('<span class="cfvsw-selected-label"></span>');
        label.children('label').children('.cfvsw-selected-label').html(swatch.data('title'));
    }
    ...
    function triggerAddToCart(variant) {
        ...
        $.ajax({
            ...
            success(response) {
                ...
                $(document.body).trigger('added_to_cart', [response.fragments, response.cart_hash, variant, ]);
    ...
    $(document).on({
        mouseenter() {
            ...
            if ('' === tooltip || 'undefined' === typeof tooltip || addToTooltip.hasClass('cfvsw-label-option')) {
                return;
            }
            if (addToTooltip.children('.cfvsw-tooltip').length === 0) {
                ...
                const swatchHeight = addToTooltip.children('.cfvsw-swatch-inner').innerHeight();
                $('.cfvsw-tooltip').css({
                    bottom: swatchHeight,
                });
                if (cfvsw_swatches_settings.tooltip_image && addToTooltip.hasClass('cfvsw-image-option')) {
                    $('.cfvsw-tooltip').prepend("<span class='cfvsw-tooltip-preview'></span>");
                    const preview = addToTooltip.children('.cfvsw-swatch-inner').css('backgroundImage');
                    $('.cfvsw-tooltip').css({
    ...
    function setSwatchesSelection() {
        $('.cfvsw-hidden-select select').each(function() {
            ...
            $(this).parent().next().find(`[data-slug='${ selected }']`).trigger('click');
        });
    }
    ...
    function resetPrice(swatch) {
        if (swatch.parents('li').find('.cfvsw-original-price').length) {
            ...
            swatch.parents('li').find('.cfvsw-original-price').removeClass('cfvsw-original-price').addClass('price');
        }
    }
